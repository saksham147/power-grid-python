"""Turns the exceptions this API raises into uniform ApiError responses.

Ported from ``Producer.api.ApiExceptionHandler``. Without it every failure would
surface as a bare 500 for cases that are plainly the caller's doing: an unknown plant
id, a malformed body, an inconsistent set of ratings.
"""
import functools
import logging
...
def handle_api_errors(view):
    """Wraps a view so the exceptions it can raise become ApiError responses -- the
    one mapping ``ApiExceptionHandler`` gives every controller in the Java service.

    Only ``ValueError`` (which covers malformed JSON) and the model's own
...
def _respond(status: HTTPStatus, message: str, details: list[str] | None = None):
    logger.debug('Responding %s: %s', status.value, message)
